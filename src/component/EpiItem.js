...
import item from "../db/episodeitem.json";
...
function Item(props){
    ...
    return(
        <div className={"epi-item "+(props.release==='no'?'disabled':"")}
                ...
                <div className="play-icon" >
                    {isover?(
                        <svg viewBox="0 0 24 24">
                            <path fill="#fff" d="M4 2.69127C4 1.93067 4.81547 1.44851 5.48192 1.81506L22.4069 11.1238C23.0977 11.5037 23.0977 12.4963 22.4069 12.8762L5.48192 22.1849C4.81546 22.5515 4 22.0693 4 21.3087V2.69127Z"></path>
                        </svg>
                    ):("")}
                </div>
            </div>
...
function EpiItem(props){
    ...
    return(
        ...
        lastlist.map((item)=>{
            return <Item key={item.index} avail={item.avail} release={item.release} index={item.index} src={item.src} title={item.title} duration={item.duration} synopsis={item.synopsis}></Item>
        })        
    )
}
